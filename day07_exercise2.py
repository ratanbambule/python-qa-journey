...
for tc in test_cases:
    ...
    tc["actual"] = actual_result   # 👈 storing actual result inside dictionary
...
# 🧾 Print final report from updated dictionary

for tc in test_cases:
    if tc["status"] == "SKIPPED":
        print(f'{tc["id"]} | Module: {tc["module"]} | Priority: {tc["priority"]} → SKIPPED')
    else:
        print(f'{tc["id"]} | Module: {tc["module"]} | Priority: {tc["priority"]} | Expected: {tc["expected"]} | Actual: {tc["actual"]} → {tc["status"]}')
